...
export default function RefereeRacesScreen() {
  ...
  const fetchRaces = async () => {
    setLoading(true);
    try {
      const data = await api.getRefereeRaces();
      setRaces(data);
    } catch (error) {
      console.error('Failed to fetch referee races', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <View className="px-5 py-4 bg-white border-b border-slate-100 shadow-sm z-10 flex-row items-center">
        <Text className="text-xl font-bold text-slate-800">Trận đấu của tôi</Text>
      </View>
      ...
      >
        {loading && races.length === 0 ? (
          ...
          <View className="py-20 items-center justify-center">
            <Gavel size={48} color="#cbd5e1" />
            <Text className="text-slate-800 font-bold text-lg mt-4">Không có trận đấu</Text>
            <Text className="text-slate-500 text-sm mt-1">Bạn không có trận đấu nào được phân công giám sát.</Text>
          </View>
        ) : (
          <View>
            <Text className="text-sm font-medium text-slate-500 mb-4 uppercase tracking-wider">
              GIÁM SÁT {races.length} TRẬN ĐẤU
            </Text>
              ...
              return (
                ...
                >
                  <View className="flex-row items-center mb-3">
                    <View className="w-10 h-10 rounded-xl bg-purple-50 items-center justify-center mr-3 border border-purple-100">
                      <Gavel size={20} color="#a855f7" />
                    </View>
                    <View className="flex-1">
                      <Text className="text-base font-bold text-slate-800 mb-1">{race.name}</Text>
