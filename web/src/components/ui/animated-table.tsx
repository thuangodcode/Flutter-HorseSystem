"use client";
import { Check, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronsUpDown, ChevronUp, Columns3, Search, X } from "lucide-react";
...
>(({ className, stickyHeader, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("overflow-auto w-full", stickyHeader && "max-h-125", className)}
    {...props}
  />
));
...
// Expanded Row
const ExpandedRow = ({ children, colSpan }: { children: React.ReactNode; colSpan: number }) => (
  <motion.tr
            ...
            <tr className="border-[var(--border)] border-b">
              ...
              {selectable && (
                <TableHead className="w-12">
                  <CheckboxCell checked={allSelected} indeterminate={someSelected} onChange={handleSelectAll} />
                </TableHead>
              )}
                    ...
                    >
                      {expandable && (
                        <TableCell className="w-10">
                          <ExpandButton isExpanded={expandedRows.has(row.id)} onClick={() => toggleRowExpanded(row.id)} />
                        </TableCell>
                      )}
                      {selectable && (
                        <TableCell className="w-12">
                          <CheckboxCell checked={selectedIds.includes(row.id)} onChange={() => handleSelectRow(row.id)} />
                        </TableCell>
                      )}
                      {displayedColumns.map((column) => (
